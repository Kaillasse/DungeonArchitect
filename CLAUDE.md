...
- `rendering/` — `world_renderer.py` (`WorldRenderer.render(screen, dungeon, camera, spawn_preview=None, hide_object_types=None, show_link_indicators=False)`: draws the tile grid with a `(tile_index, zoom)` surface cache, placed objects sized from `OBJECT_TYPES[type]["size"]` (in grid cells, not raw sprite pixels), debug grid lines, the spawn-preview marker, and (only when `show_link_indicators=True`, which only `Creator` passes) each linkable object's indicator dot and any persisted links. Holds no world state itself — only pygame surface caches).
...
**Button/gate/wall linking — editor side done, runtime behavior not started:**
...
- **Still not implemented:** actually acting on a link during `EXPLORATION` — stepping on a `button` should trigger its own animation plus its linked `gate`/`wall`'s open animation and make that cell walkable. `logical_grid` today only encodes static `EMPTY`/`FLOOR`/`WALL`, with no notion of a cell becoming walkable/unwalkable at runtime without a full `rebuild()` — this needs some runtime toggle state layered on top before the button/gate interaction itself can be written.
