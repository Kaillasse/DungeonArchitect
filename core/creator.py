...
import core
from core.grid import DEFAULT_GRID_SAVE_PATH, DungeonGrid
from core.ui import ToolPaletteUI
...
from core.tools import ObjectTool
...
class Creator:
    ...
    def __init__(self, game_manager):
        ...
        self.room_manager = RoomManager(self.editor)
        self.editor.load_from_json()
    ...
    def _mouse_to_grid(self, mouse_pos):
        ...
        return self.editor.world_to_grid(world_x, world_y)
    ...
    def _is_valid_grid_cell(self, mouse_pos):
        ...
        return (
            0 <= grid_x < self.editor.width
            and
            0 <= grid_y < self.editor.height
        )
    ...
    def _try_place_object(self):
        ...
        grid_x, grid_y = self.editor.world_to_grid(
            *world
        )
    ...
    def run(self):
        ...
        if os.environ.get("DUNGEONARCHITECT_HEADLESS") == "1":
            self.editor.save_to_json(DEFAULT_GRID_SAVE_PATH)
        ...
        while running:
            ...
            if self.object_tool.dragging:
                ...
                if (
                    ...
                    if self.editor.logical_grid[grid_y][grid_x] == FLOOR:
        ...
        self.editor.save_to_json(self.room_manager.current_room)